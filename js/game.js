...
/* =========================================================================
   ...
   Solo-Kampagne (3 Wochen + Boss) und Online-Duell (2 Wellen + Showdown)
   ========================================================================= */
...
function freshState(mode, name, seed) {
  return {
    mode, name,
    seed,
    deck: [],            // Array von Abschnitten (Wochen bzw. Wellen), je Array von Fällen
    sections: [],        // Konfiguration je Abschnitt {title, intro, energy, timer, nr}
    ...
    lastEnergyBonus: 0,
    crisis: false,
    boss: null,          // {feed, fakeIdx, energy, timeLeft, done, build}
    duel: null,          // {oppName, oppScore, oppIndex, oppBuild, oppHunt, oppFinal, myBuild, myHunt, dropped}
...
function showCase() {
  ...
  $("hud-week").textContent = G.mode === "duel" ? "WELLE " + (G.secIdx + 1) : "WOCHE " + (G.secIdx + 1);
...
/* =========================================================================
   DILEMMA-UPGRADES (nur Solo, nach Woche 1 & 2)
   ========================================================================= */
...
function finishBuild(timeout) {
  stopTimer();
  // Unvollständige Auswahl (Timeout): Rest zufällig auffüllen
...
function afterHuntReveal() {
  ...
  // Duell: auf Jagd-Ergebnis des Gegners warten (Angreifer-Bonus!)
...
function showResult() {
  ...
  $("result-breakdown").innerHTML = `
    ...
    <div><span>Multiplikator</span><span>× ${mult.toFixed(2)}</span></div>
...
/* =========================================================================
   RANGLISTE (localStorage)
   ========================================================================= */
function loadBoard() {
  try { return JSON.parse(localStorage.getItem("ww_board_v1")) || []; }
  catch (e) { return []; }
}
...
function saveToBoard() {
  ...
  board.push({ name: G.name, score: G.finalScore, acc, index: G.index, mode: G.mode, date: new Date().toISOString().slice(0, 10) });
  board.sort((a, b) => b.score - a.score);
  localStorage.setItem("ww_board_v1", JSON.stringify(board.slice(0, 10)));
}
...
function renderBoard() {
  ...
  if (!board.length) {
    wrap.innerHTML = `<div class="board-empty">Noch keine Einträge. Spiel eine Kampagne – dann steht dein Name hier.</div>`;
  } else {
    wrap.innerHTML = board.map((e, i) => `
      <div class="board-row">
        <span class="pos">${i + 1}.</span>
        <span class="bname">${esc(e.name)} <span class="bmeta">${e.mode === "duel" ? "⚔️ Duell" : "🛡️ Solo"} · ${e.acc} % · Index ${e.index} · ${e.date}</span></span>
...
function openLobby() {
  ...
  if (typeof Peer === "undefined") {
    ...
    return lobbyError("Die Netzwerk-Bibliothek (PeerJS) konnte nicht geladen werden. Bist du offline? Für das Online-Duell wird eine Internetverbindung benötigt.");
  }
...
function wireNet() {
  ...
  Net.onConnected = () => {
    if (!Net.isHost) {
      $("lobby-status").textContent = "Verbunden! Warte auf Spielstart…";
      $("lobby-choice").classList.add("hidden");
      $("lobby-wait").classList.remove("hidden");
      $("room-code-display").textContent = "OK";
      Net.send("hello", { name: myName || "Gast" });
    } else {
      $("lobby-status").textContent = "Gegner verbunden! Starte Duell…";
    }
  };
  ...
  Net.onDropped = (reason) => {
    if (!G || G.mode !== "duel" || !G.duel) {
      // Abbruch schon in der Lobby (vor Spielstart)
      $("lobby-choice").classList.remove("hidden");
      $("lobby-wait").classList.add("hidden");
    ...
    // Falls wir gerade auf den Gegner warten: mit HYDRA-Ersatz weitermachen
    if (!$("overlay-wait").classList.contains("hidden")) {
      ...
      if (G.duel.myBuild && !G.duel.oppBuild) {
        ...
        showDuelResult();   // zeigt ohne Gegner-Endstand das Solo-Ergebnis
      }
    }
  };
...
function finishDuel() {
  showOverlay("overlay-wait", false);
  // Angreifer-Bonus: Mein Fake blieb beim Gegner unentdeckt
...
function showDuelResult() {
  ...
  if (!opp) {  // Gegner weg -> Solo-Ergebnis zeigen
    return showResult();
  }
...
function init() {
  ...
  $("btn-join-room").addEventListener("click", () => {
    ...
    $("lobby-wait").classList.remove("hidden");
    $("room-code-display").textContent = code;
    $("lobby-status").textContent = "Verbinde…";
    Net.joinRoom(code);
  });
  ...
  $("btn-board-clear").addEventListener("click", () => {
    if (confirm("Rangliste wirklich leeren?")) { localStorage.removeItem("ww_board_v1"); renderBoard(); }
  });
